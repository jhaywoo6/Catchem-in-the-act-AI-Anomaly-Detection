...
def init_parser(default_data_dir='data/', default_exp_dir='data/exp_dir'):
    ...
    # Model Params
    parser.add_argument('--rnn_hidden_dim', type=int, default=64, metavar='HD', help='Hidden dimension of RNN')
    parser.add_argument('--rnn_num_layers', type=int, default=2, metavar='NL', help='Number of RNN layers')
    parser.add_argument('--rnn_output_dim', type=int, default=1, metavar='OD', help='Output dimension of RNN')
...
def init_rnn_params(args, dataset):
    """
    Function to initialize the parameters for the RNN model from the args.
    """
    # You can add any necessary logic for dataset-specific arguments here
    # For example, setting different parameters based on the dataset, if needed
    # Here, we directly extract the parameters
    
    rnn_params = {
        'pose_shape': (14, 10, 2),  # or use `args.pose_shape` if pose shape is passed as an argument
        'hidden_dim': args.rnn_hidden_dim,  # Assuming you add rnn_hidden_dim in the args file
        'num_layers': args.rnn_num_layers,  # Similar for num_layers
        'output_dim': args.rnn_output_dim,  # You can define rnn_output_dim in args
    }
