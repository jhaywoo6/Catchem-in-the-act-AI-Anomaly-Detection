...
from models.STG_NF.model_pose import SimpleRNNModel
from models.training import Trainer
from utils.data_utils import trans_list
from utils.optim_init import init_optimizer, init_scheduler
from args import create_exp_dirs
from args import init_parser, init_sub_args, init_rnn_params
...
def main():
    ...
    #rnn_args = init_rnn_params(args, dataset)
    #model = SimpleRNNModel(**rnn_args)
    #num_of_params = calc_num_of_params(model)
    if not torch.cuda.is_available() or torch.cuda.device_count() <= int(args.device.split(':')[-1]):
    ...
    auc_roc, scores_np, auc_pr, eer, eer_threshold,precision,recall,accuracy = score_dataset(normality_scores, dataset["test"].metadata, args=args)
