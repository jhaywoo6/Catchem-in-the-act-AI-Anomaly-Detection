...
class RNNModel(nn.Module):
    ...
    def forward(self, x, label=None, score=None):
        rnn_out, _ = self.rnn(x)  # [batch, seq_len, hidden_dim*num_directions]

        # Attention over time
        attn_weights = torch.softmax(self.attn(rnn_out).squeeze(-1), dim=1)  # [batch, seq_len]
        attn_output = torch.sum(rnn_out * attn_weights.unsqueeze(-1), dim=1)  # [batch, hidden_dim*num_directions]
