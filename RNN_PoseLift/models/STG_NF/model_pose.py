...
class FlowStep(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # Dynamically set device
        print(self.device)
...
class STG_NF(nn.Module):
    def __init__(
            ...
            strategy='uniform',
            max_hops=8,
            device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # Default to None
    ):
        super().__init__()
        self.device = device or torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # Dynamically set device
        self.flow = FlowNet(
...
class SimpleRNNModel(nn.Module):
    def __init__(self, pose_shape, hidden_dim, num_layers, output_dim):
        super(SimpleRNNModel, self).__init__()

        # Assuming pose_shape is (C, T, V) where C is keypoints, T is frames, V is coordinates
        self.input_size = pose_shape[0] * pose_shape[2]  # C * V
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.output_dim = output_dim

        # Define the RNN layer
        self.rnn = nn.RNN(input_size=432, hidden_size=self.hidden_dim,
    ...
    def forward(self, x):
        ...
        x = x.contiguous().view(batch_size, T, C * V)  # [batch, T, input_size]
        
        out, _ = self.rnn(x)  # Input now 3D
        output = self.fc(out[:, -1, :])  # e.g., final timestep output
        return output