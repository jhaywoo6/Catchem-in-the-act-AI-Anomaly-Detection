...
def score_dataset(score, metadata, args=None, save_results=True, directory="results/csv_files/"):
    ...
    # Calculate AUC-ROC, AUC-PR, EER and EER threshold
    auc_roc, auc_pr, eer, eer_threshold = score_auc(scores_np, gt_np)

    # Binarize predictions using EER threshold
    pred_binary = (scores_np >= eer_threshold).astype(int)

    # Compute precision, recall, accuracy
    precision = precision_score(gt_np, pred_binary)
    recall = recall_score(gt_np, pred_binary)
    accuracy = accuracy_score(gt_np, pred_binary)

    return auc_roc, scores_np, auc_pr, eer, eer_threshold, precision, recall, accuracy
