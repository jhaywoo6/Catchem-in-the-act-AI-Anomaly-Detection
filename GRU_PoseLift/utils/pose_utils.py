...
def split_pose_to_segments(single_pose_np, single_pose_meta, single_pose_keys, start_ofst=0, seg_dist=6, seg_len=12,
    ...
    pose_segs_meta = []
    num_segs = np.ceil((clip_t - seg_len) / seg_dist).astype(int)  # Replace np.int with int
    single_pose_keys_sorted = sorted([int(i) for i in single_pose_keys])  # Ensure keys are integers
