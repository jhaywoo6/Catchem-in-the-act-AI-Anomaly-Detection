...
class GRUModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim=1, num_layers=2, dropout=0.3, bidirectional=True):
    ...
    def forward(self, x, label=None, score=None):
        gru_out, _ = self.gru(x)  # [batch, seq_len, hidden_dim*num_directions]

        # Attention over time
        attn_weights = torch.softmax(self.attn(gru_out).squeeze(-1), dim=1)  # [batch, seq_len]
        attn_output = torch.sum(gru_out * attn_weights.unsqueeze(-1), dim=1)  # [batch, hidden_dim*num_directions]

        attn_output = self.norm(attn_output)
        attn_output = self.dropout(attn_output)
        logits = self.fc(attn_output).squeeze(-1)

        nll = self.loss_fn(logits, label.float()) if label is not None else None
        return logits, nll
...
class Trainer:
    def __init__(self, args, model, train_loader, test_loader,
        ...
        self.scheduler = scheduler_f(self.optimizer) if scheduler_f else None
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, data_arr in enumerate(pbar):
                try:
                    ...
                    label = data[-1]
                    samp = data[0] if self.args.model_confidence else data[0][:, :self.args.input_dim]
            ...
            print('Checkpoint Saved. New LR: {:.3e}'.format(new_lr))
